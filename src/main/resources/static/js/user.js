            ...
            <div class="news-header">
                ...
                ${withInteractions ? `
                    <div class="topic-actions">
                        ...
                        <button class="btn" onclick="showNewsComments(${item.id})">💬 Коментарі</button>
                    </div>
                ` : ''}
            </div>
        </article>
...
function filterAndRenderMatches(resetPage = false) {
    ...
    if (currentFilteredMatches.length === 0) {
        ...
        document.getElementById('load-more-matches').style.display = 'none';
        return;
    }
    ...
    try {
        ...
        if (typeof renderTeamsList === 'function') {
            ...
            if (combined.length === 0) {
                ...
                <div class="team-card">
                    <div class="team-icon"><div class="team-emblem">🏆</div></div>
        ...
        updateStatistics('teams', totalCount);
    } catch (error) {
        console.error('Помилка:', error);
...
function showNewsComments(newsId) {
    showMessage('Функція коментування новин буде доступна незабаром', 'success');
}
